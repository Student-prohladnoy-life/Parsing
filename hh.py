...
# Готовим нунжые переменные. Затем проходим по кажэдой из получиных страниц
...
for page in range(count_pages):
    ...
    for res in ress['items']:
        ...
        # Добавление города из сапросса, если его нет в файле
        ...
        # print(pp)
        pp_re = re.findall(r'\s[A-Z-a-z-?]+', pp)
        # print(pp_re)
        ...
        for it in its:
            if not any(it in x for x in skills):
                skillis.append(it)
        # околнчатильное формирование списка навыков
        # обработка зарплаты
        if res_full['salary']:
